...
Defined in `.claude/commands/` (local) and `routines/` (cloud). Eight scheduled runs per market per day (5 core + pulse + watchlist-approve + monthly-recal on first-Sat) plus two ad-hoc helpers.
