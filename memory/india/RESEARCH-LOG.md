...
- APPROVAL (Sai, in-session 2026-07-19 evening): orb_test_sleeve v4 proposal ACCEPTED at 0.75x for first 20 live trades. TRADING-STRATEGY.md v4 section written (universe 149, no width gate, entries 09:30-11:30, trail 1x width, bank-week Rs1500, limit orders only, Rs560 risk cap). G14 wired into /trade-india; scanner v4 default (--legacy for old v3). BLOCKER for Monday: DHAN_ACCESS_TOKEN expired - must refresh before 09:15 or no orders can be placed.
- Recommendation-flow fix (Sai request): scan_all.py (10-min Telegram scan) was still v3 - MID50 disabled, width>=1.5% gate, ENTRY required VWAP+RSI. Now v4: 149 tickers, 0.10% floor only, volume-only gate (VWAP/RSI advisory tags), ENTRY-LATE tag after 11:30, ORB WATCH section (top-5 near-signals) in every Telegram message, and every actionable ORB ENTRY auto-journaled (dedup id ORB-DATE-TICKER-SIDE). Expect several-fold more visible signals.
- OBSERVATION MODE set (Sai): 2026-07-20 -> 2026-07-22 NO Dhan orders; monitor signal quality + journal entries; live entries resume 2026-07-23. Gate 0 added to /trade-india.
- ORB failure study (1,046 trades, 81 wks): losing-steam signatures quantified. VWAP rejection in first 30 min post-entry -> 90.4% loss rate (vs 43.4% held); fell back inside box within 2 bars -> 80%; breakout-bar volume collapse (<0.35x next 3 bars) -> 56% vs 37.9% sustained. Entry time/gap/width mostly flat (late 11:00-11:30 entries actually BEST at 42.7% loss). backtests/orb_failure_study.py.
- NEW: journal/orb_autopsy.py - nightly EOD autopsy of every journaled ORB signal (real or observation-mode paper). Replays v4 execution on 5-min bars (Kotak cache preferred), attributes primary cause from the evidence-based taxonomy above, appends failure blocks to POST-MORTEMS.md + orb_autopsy.jsonl. Wired into run_journal_update.bat. Future proposal candidate once N>=30 live: early exit on VWAP rejection.
